...
/**
 * Reserved operator identity on BASE. Tenant-bound rows are excluded so a SaaS workspace that
 * creates its own "Mike Strickland" can neither sign in here nor shadow the real operator row.
 */
function basePlatformOperatorLoginSql(alias) {
  ...
  return `(
    ...
    AND LOWER(BTRIM(COALESCE(${a}.portal_files_client_id, ''))) NOT LIKE 'tenant-%'
  )`;
}
...
async function userBelongsToSaasTenantBox(pool, userId, userRow = null) {
  ...
  if (!userRow) {
    const identity = await pool.query(
      `SELECT username, display_name, email, portal_files_client_id FROM users WHERE CAST(id AS text) = $1 LIMIT 1`,
      [uid]
    );
...
async function assertLoginEnvironmentAccess(pool, userRow, requestHost) {
  ...
  /** Platform operator roams every host; a tenant's own "Mike" is not the platform operator. */
  if (!tenantBound && looksLikeMike(userRow)) {
    return { allowed: true };
  }
 ...
 * @returns {Promise<{ ok: true } | { ok: false, error: string, existingUserId?: string, existingUsername?: string }>}
 */
async function assertUsernameAvailableForCreate(pool, username, scope) {
  ...
  const r = await pool.query(
    `SELECT id, username, display_name, portal_files_client_id
     FROM users
  ...
  if (r.rows.length) {
    const row = r.rows[0];
    const shown = cleanString(row.display_name) || cleanString(row.username) || name;
    if (scope?.mode === 'tenant') {
      return {
        ok: false,
        error: `Username already exists in your workspace: ${shown}`,
        existingUserId: String(row.id),
        existingUsername: cleanString(row.username) || name
      };
    }
    return {
      ok: false,
...
/**
 * Same portal-scope uniqueness rule as Postgres, for in-memory snapshot user rows.
 * @param {object[]} users
 * @param {string} username
 * @param {{ mode?: string, portalClientId?: string } | null | undefined} scope
 * @returns {object | null}
 */
function findSnapshotUsernameConflict(users, username, scope) {
  const usernameLower = cleanString(username).toLowerCase();
  if (!usernameLower || !Array.isArray(users)) return null;
  const scopeKey = scope?.mode === 'tenant' ? usernamePortalScopeKey(scope.portalClientId) : '__global__';
  return (
    users.find((u) => {
      if (cleanString(u?.username).toLowerCase() !== usernameLower) return false;
      return usernamePortalScopeKey(u?.portal_files_client_id || u?.portalFilesClientId) === scopeKey;
    }) || null
  );
}
