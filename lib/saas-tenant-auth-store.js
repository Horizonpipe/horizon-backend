...
async function seedTenantAuthSnapshot(slug, ownerUser) {
  ...
  if (existing && Array.isArray(existing?.data?.users) && existing.data.users.length) {
    return { ok: true, key: authSnapshotKey(slug), skipped: true };
  }
