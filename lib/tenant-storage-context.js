...
async function resolveTenantStorageContext(pool, userId, options = {}) {
  ...
  if (profile.features.tenantHostBindingStrict) {
    // Subdomain labels strip spaces (techpipe) while Wasabi storage slugs hyphenate (tech-pipe).
    // Match either exact slug or compact (hyphen-insensitive) form.
    const compact = (s) => String(s || '').replace(/-/g, '');
    const hostOk =
      !!hostSlug &&
      !!tenantSlug &&
      (hostSlug === tenantSlug || compact(hostSlug) === compact(tenantSlug));
    if (!hostOk) {
      return { ...EMPTY_CONTEXT };
    }
