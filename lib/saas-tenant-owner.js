...
const { SAAS_OWNER_ROLES, SAAS_PURCHASER_BASELINE_ROLES } = require('./saas-owner-roles');
...
async function loadOwnerUserRow(pool, userId) {
  ...
  const r = await pool.query(
    `SELECT id, username, display_name, email, portal_files_client_id FROM users WHERE CAST(id AS text) = $1 LIMIT 1`,
    [uid]
  );
